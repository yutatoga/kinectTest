...
void ofApp::draw(){
    ...
    // draw mesh
    // ofRect(540, 200, 1920/4, 1080/4);
    int w = rawDepthPixels.getWidth();
    int h = rawDepthPixels.getHeight();
    ofMesh mesh;
    mesh.setMode(OF_PRIMITIVE_POINTS); // FIXME: set OF_PRIMITIVE_TRIANGLES
    int step = 2;
    for (int y = 0; y < h; y += step) {
        for (int x = 0; x < w; x += step) {
            mesh.addVertex(ofVec3f(x, y, rawDepthPixels[x+y*rawDepthPixels.getWidth()]));
        }
    }
